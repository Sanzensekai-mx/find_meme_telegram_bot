...
async def process_media_send(mes, state):
    data_from_state = await state.get_data()
    if mes.photo:
        data_from_state.get('list_of_img_id').append(InputMediaPhoto(mes.photo[-1].file_id, caption=mes.caption))
    elif mes.video:
        # print(mes.video)
        data_from_state.get('list_of_img_id').append(InputMediaVideo(mes.video.file_id, caption=mes.caption))
    await state.update_data(data_from_state)
    await state.reset_state(with_data=False)
    await AdminMailing.Media.set()


@dp.message_handler(chat_id=admins, state=AdminMailing.Media, content_types=ContentType.all())
async def send_group_photo(message: Message, state: FSMContext):
    data_from_state = await state.get_data()
    if data_from_state.get('list_of_img_id') is None:
        await state.update_data({'list_of_img_id': []})
    else:
        print(data_from_state.get('list_of_img_id'))
    if message.photo or message.video:
        ...
        with open(os.path.join(os.getcwd(), 'data', 'user_info.json'), 'r', encoding='utf-8') as user_r:
            ...
            for user_chat_id in users_chat_id:
                try:
                    await bot.send_media_group(chat_id=user_chat_id, media=data_from_state.get('list_of_img_id'))
                    await sleep(0.3)
