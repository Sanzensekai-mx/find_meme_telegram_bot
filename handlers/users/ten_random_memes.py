...
async def process_random_memes(state, mes=None, call=None):
    await mes.answer('Ваши рандомные мемы, сэр',
                     reply_markup=cancel_ten_random) if mes is not None \
        else await call.message.answer('Ваши рандомные мемы, сэр',
                                       reply_markup=cancel_ten_random)
    ...
    while len(result_random) < 10:
        result_random.add(choice(mem_data))
    ...
    for num, res in enumerate(result_random, 1):
        ...
        state_data.get('result_mem_search_by_page')[1].update({str(num): res.meme_name})
        result_kb.insert(res_button)
        result_message += f'{num}. {res.meme_name}\n\n'
    result_kb.add(InlineKeyboardButton('Еще 10 мемов',
...
async def start_ten_random_memes(message: Message, state: FSMContext):
    ...
    # LOG you!!!!!!!
    logging.info(f'from: {message.chat.first_name}, '
                 f'text: {message.text}')
    # LOG you!!!!!!!
