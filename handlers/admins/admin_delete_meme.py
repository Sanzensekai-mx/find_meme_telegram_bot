...
async def cancel_del_meme(message: Message, state: FSMContext):
    logging.info(f'from: {message.chat.full_name}, '
                 f'text: {message.text}, info: Отмена добавления нового мема.')
    await message.answer('Отмена удаления мема.',
...
async def start_del_meme(message: Message):
    ...
    await message.answer('Введите точное имя мема для удаления из БД.'
                         '\nИли /cancel_del_meme для отмены удаления.',
                         reply_markup=admin_cancel_del_meme)
...
async def enter_del_meme_name(message: Message, state: FSMContext):
    ...
    if await db.get_meme(name):
        ...
        await message.answer(f'Мем "{name}" был удален.',
                             reply_markup=main_menu)
        await state.finish()
    else:
